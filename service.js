...
//#endregion
//#region node_modules/idb-ts/lib/index.esm.js
init_tslib_es6();
require_Reflect();
...
var QueryBuilder = class QueryBuilder {
	...
	addNestedGroup(builder) {
		var _a;
		const nested = new QueryBuilder(this.db, this.storeName, this.transaction);
		const returned = (_a = builder(nested)) !== null && _a !== void 0 ? _a : nested;
		const clauses = returned instanceof QueryBuilder ? returned.clauses : nested.clauses;
		this.appendClause({
			kind: "group",
			clauses
		});
	...
	requireCurrentField(operation) {
		if (!this.currentField) throw new Error(`No field specified for ${operation}`);
	...
	createReadRequest(store) {
		if (!this.indexName) return store.getAll();
		if (!store.indexNames.contains(this.indexName)) throw new Error(`Index '${this.indexName}' does not exist on ${this.storeName}`);
		const index = store.index(this.indexName);
		let keyRange;
		if (this.rangeStart !== void 0 && this.rangeEnd !== void 0) keyRange = IDBKeyRange.bound(this.rangeStart, this.rangeEnd);
		else if (this.rangeStart !== void 0) keyRange = IDBKeyRange.lowerBound(this.rangeStart);
		else if (this.rangeEnd !== void 0) keyRange = IDBKeyRange.upperBound(this.rangeEnd);
		return keyRange ? index.getAll(keyRange) : index.getAll();
	}
	matchesClause(item, clause) {
		if (clause.kind === "group") return this.evaluateClauses(item, clause.clauses);
		...
		switch (clause.op) {
			...
			case "endsWith": return typeof value === "string" && value.endsWith(String(clause.value));
			case "contains":
				if (typeof value === "string") return value.includes(String(clause.value));
			...
			case "notIn": return Array.isArray(clause.value) && !clause.value.includes(value);
			case "containsAny": return Array.isArray(value) && Array.isArray(clause.value) && clause.value.some((entry) => value.includes(entry));
			case "containsAll": return Array.isArray(value) && Array.isArray(clause.value) && clause.value.every((entry) => value.includes(entry));
		}
	}
	...
	collectMatches() {
		return __awaiter(this, void 0, void 0, function* () {
			return (yield this.loadCandidates()).filter((item) => this.evaluateClauses(item, this.clauses));
		});
	}
	sortResults(results) {
		if (!this.orderField) return results;
	...
	applyPagination(results) {
		...
		if (this.offsetCount !== void 0) nextResults = nextResults.slice(this.offsetCount);
		if (this.limitCount !== void 0) nextResults = nextResults.slice(0, this.limitCount);
		return nextResults;
	}
	aggregateValues(field, reducer) {
		return __awaiter(this, void 0, void 0, function* () {
			const results = yield this.collectMatches();
			return reducer(field ? results.map((item) => item[field]) : results);
		});
	}
	...
	count() {
		return __awaiter(this, void 0, void 0, function* () {
			if (this.groupField) return this.aggregateGroupedCount(this.groupField);
			return (yield this.collectMatches()).length;
		});
	}
	sum(field) {
		return __awaiter(this, void 0, void 0, function* () {
			return yield this.aggregateValues(field, (values) => values.reduce((accumulator, value) => accumulator + (Number(value) || 0), 0));
		});
	}
	avg(field) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			return values.reduce((accumulator, value) => accumulator + (Number(value) || 0), 0) / values.length;
		});
	}
	min(field) {
		return __awaiter(this, void 0, void 0, function* () {
...
function Index(options) {
	return (target, propertyKey) => {
		const constructor = target.constructor;
		const nextIndexes = [...Reflect.getMetadata("indexes", constructor) || [], {
			field: propertyKey,
			options
		}];
...
function DataClass(options = {}) {
	return (target) => {
		if (!Reflect.getMetadata("keypath", target)) throw new Error(`No keypath field defined for the class ${target.name}.`);
		if ((Reflect.getMetadata("individual_keypaths", target) || []).length > 1) throw new Error(`Only one keypath field can be defined for the class ${target.name}.`);
...
var Database = class Database {
	constructor(dbName, classes, printEnabled = false) {
		...
		if (!classes.every((cls) => Reflect.getMetadata("dataclass", cls))) throw new Error("All classes should be decorated with @DataClass.");
		...
		this.retentionPolicies = this.classes.map((cls) => {
			...
			if (!(policy === null || policy === void 0 ? void 0 : policy.enabled)) return null;
			return {
				className: cls.name,
			...
			return new Promise((resolve, reject) => {
				...
				request.onupgradeneeded = (event) => {
					...
					this.classes.forEach((cls) => {
						...
						if (classVersion > oldVersion) if (!db.objectStoreNames.contains(storeName)) {
							this.printDebug(`Creating object store: ${storeName} (version ${classVersion})`);
							const storeOptions = {};
							if (keyPathMetadata) {
								storeOptions.keyPath = keyPathMetadata.fields;
								if ((_a = keyPathMetadata.options) === null || _a === void 0 ? void 0 : _a.autoIncrement) storeOptions.autoIncrement = true;
							}
							...
							indexFields.forEach((indexField) => {
								...
								const indexOptions = typeof indexField === "string" ? { unique: false } : (_a = indexField.options) !== null && _a !== void 0 ? _a : { unique: false };
								if (!store.indexNames.contains(indexName)) store.createIndex(indexName, indexName, indexOptions);
							});
						} else {
	...
	calculateRetentionCleanupIntervalMs() {
		if (!this.retentionPolicies.length) return;
		...
		return this.retentionPolicies.map(({ policy }) => policy.seconds).reduce((accumulator, value) => gcd(accumulator, value)) * 1e3;
	}
	startRetentionCleanup() {
		const cleanupIntervalMs = this.calculateRetentionCleanupIntervalMs();
		if (!cleanupIntervalMs || !this.db || this.retentionTimer) return;
	...
	runRetentionCleanup() {
		return __awaiter(this, void 0, void 0, function* () {
			if (!this.db || this.retentionCleanupRunning || !this.retentionPolicies.length) return;
			this.retentionCleanupRunning = true;
			try {
				this.printDebug("Retention cleanup tick started");
				for (const { storeName, className, policy } of this.retentionPolicies) yield this.cleanupExpiredRecords(storeName, className, policy);
	...
	cleanupExpiredRecords(storeName, className, policy) {
		...
		return new Promise((resolve, reject) => {
			try {
				...
				request.onsuccess = () => {
					...
					if (typeof timestamp === "number" && timestamp <= cutoff) {
						...
						deleteRequest.onerror = () => {
							var _a;
							return reject((_a = deleteRequest.error) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new Error(`Retention cleanup delete failed for ${className}`));
						};
						return;
					}
					cursor.continue();
				...
				transaction.onerror = () => {
					var _a;
					return reject((_a = transaction.error) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new Error(`Retention cleanup failed for ${className}`));
				};
				transaction.onabort = () => {
					var _a;
					return reject((_a = transaction.error) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new Error(`Retention cleanup aborted for ${className}`));
				};
			} catch (error) {
				reject(error);
			}
	...
	createEntityRepository(cls, transaction) {
		...
		const validateItem = (item) => {
			...
			validators.forEach((rule) => {
				...
				if (!valid) failures.push(`${rule.field}: ${rule.message}`);
			});
			if (failures.length) throw new Error(`Validation failed for ${cls.name}: ${failures.join("; ")}`);
		};
		const generateKey = (item) => {
			var _a;
			const keyPathMetadata = Reflect.getMetadata("keypath", cls);
			if (!((_a = keyPathMetadata === null || keyPathMetadata === void 0 ? void 0 : keyPathMetadata.options) === null || _a === void 0 ? void 0 : _a.generator)) return void 0;
			const generator = keyPathMetadata.options.generator;
			if (typeof generator === "function") return generator(item);
			switch (generator) {
				case "uuid": return KeyGenerators.uuid();
				case "timestamp": return KeyGenerators.timestamp();
				case "random": return KeyGenerators.random();
				default: return;
			}
		};
		...
		const updateStoredItem = (store, item) => __awaiter(this, void 0, void 0, function* () {
			...
			let existingItem;
			if (key !== void 0 && key !== null) existingItem = yield readExistingItem(store, key);
		...
		const extractKey = (item) => {
			const keyPathMetadata = Reflect.getMetadata("keypath", cls);
			if (!keyPathMetadata) return void 0;
			const fields = keyPathMetadata.fields;
			if (Array.isArray(fields)) return fields.map((field) => item[field]);
			else return item[fields];
		};
		const setKey = (item, key) => {
			const keyPathMetadata = Reflect.getMetadata("keypath", cls);
			if (!keyPathMetadata) return;
			const fields = keyPathMetadata.fields;
			if (typeof fields === "string") item[fields] = key;
		};
		return {
			query() {
				if (!self.db) throw new Error("Database not initialized.");
				const storeName = cls.name.toLowerCase();
				return new QueryBuilder(self.db, storeName, transaction);
			},
			create: (item) => __awaiter(this, void 0, void 0, function* () {
				...
				if (((_a = keyPathMetadata === null || keyPathMetadata === void 0 ? void 0 : keyPathMetadata.options) === null || _a === void 0 ? void 0 : _a.generator) && !keyPathMetadata.options.autoIncrement) {
					...
					if (currentKey === void 0 || currentKey === null || currentKey === "") {
						...
						if (generatedKey !== void 0) setKey(item, generatedKey);
					}
				}
				validateItem(item);
			...
			createMany: (items) => __awaiter(this, void 0, void 0, function* () {
				...
				for (const item of items) yield repository.create(item);
			}),
			...
			updateMany: (items) => __awaiter(this, void 0, void 0, function* () {
				const repository = this.createEntityRepository(cls, transaction);
				for (const item of items) yield repository.update(item);
			}),
			...
			deleteMany: (keys) => __awaiter(this, void 0, void 0, function* () {
				const repository = this.createEntityRepository(cls, transaction);
				for (const key of keys) yield repository.delete(key);
			}),
			deleteWhere: (predicate) => __awaiter(this, void 0, void 0, function* () {
				var _a;
				const repository = this.createEntityRepository(cls, transaction);
				const query = repository.query();
				const keys = (yield ((_a = predicate(query)) !== null && _a !== void 0 ? _a : query).execute()).map((item) => extractKey(item)).filter((key) => key !== void 0 && key !== null);
			...
			listPaginated: (page, pageSize) => __awaiter(this, void 0, void 0, function* () {
				return this.performOperation(cls.name, "readonly", (store) => {
					...
					return new Promise((resolve, reject) => {
						request.onsuccess = () => {
							const paginatedItems = request.result.slice((page - 1) * pageSize, page * pageSize);
			...
			findByIndex: (indexName, value) => __awaiter(this, void 0, void 0, function* () {
				return this.performOperation(cls.name, "readonly", (store) => {
					if (!store.indexNames.contains(indexName)) throw new Error(`Index '${indexName}' does not exist on ${cls.name}`);
	...
	performOperation(className, mode, operation, transaction) {
		return __awaiter(this, void 0, void 0, function* () {
			if (!this.db && !transaction) throw new Error("Database not initialized.");
			const storeName = className.toLowerCase();
			return operation((transaction !== null && transaction !== void 0 ? transaction : this.db.transaction(storeName, mode)).objectStore(storeName));
		});
	}
	createTransactionHandle(entityNames, mode) {
		...
		const storeNames = uniqueEntityNames.map((entityName) => {
			...
			if (!entityClass) throw new Error(`Entity '${entityName}' is not registered in ${this.dbName}.`);
		...
		const completion = new Promise((resolve, reject) => {
			...
			nativeTransaction.onabort = () => {
				...
				reject((_a = nativeTransaction.error) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new Error("Transaction aborted."));
			};
			nativeTransaction.onerror = () => {
				...
				reject((_a = nativeTransaction.error) !== null && _a !== void 0 ? _a : /* @__PURE__ */ new Error("Transaction failed."));
			};
		});
		const handle = {};
		uniqueEntityNames.forEach((entityName) => {
			const entityClass = this.classes.find((cls) => cls.name === entityName);
			if (entityClass) handle[entityName] = this.createEntityRepository(entityClass, nativeTransaction);
		});
		handle.commit = () => __awaiter(this, void 0, void 0, function* () {
			if (typeof nativeTransaction.commit === "function") nativeTransaction.commit();
			yield completion;
		});
...
//#endregion
//#region src/service/db/vault.entity.ts
var VaultType = { GDRIVE: "GDRIVE" };
var Vault = class Vault {
	id;
	name;
	type;
	createdAt;
...
__decorate([KeyPath(), __decorateMetadata("design:type", String)], Vault.prototype, "id", void 0);
__decorate([Index(), __decorateMetadata("design:type", Number)], Vault.prototype, "createdAt", void 0);
Vault = __decorate([DataClass({ version: 2 })], Vault);
/**
* @license
...
*/
//#endregion
//#region src/service/db/authorizations.entity.ts
...
var AuthorizationType = { GOOGLE: "GOOGLE" };
var Authorization = class Authorization {
	id;
	name;
	type;
	createdAt;
...
__decorate([KeyPath(), __decorateMetadata("design:type", String)], Authorization.prototype, "id", void 0);
Authorization = __decorate([DataClass({ version: 3 })], Authorization);
//#endregion
//#region src/service/app.db.ts
var appDb = {};
var ENTITIES = [Vault, Authorization];
var openDb = (0, import_lodash.once)(async () => {
	console.log(/* @__PURE__ */ new Date(), "opening DB", ENTITIES);
	const db = await Database.build("wowfs", [Vault, Authorization]);
	appDb.connection = db;
	console.log(/* @__PURE__ */ new Date(), "DB opened", db);
});
//#endregion
//#region src/service/app.routing.ts
var route = (method, url) => `${method}$|$${url}`;
var BASE_URL$1 = "/test-pages/".endsWith("/") ? "/test-pages/".slice(0, -1) : "/test-pages/";
async function performRouting(controllers, request) {
	const url = new URL(request.url);
	console.log("Routing", url.pathname, url.pathname.replace(BASE_URL$1, ""));
	const routeKey = route(request.method, url.pathname.replace(BASE_URL$1, ""));
	for (const controller of controllers) {
		const handler = controller[routeKey];
		if (handler) try {
			await openDb();
			return await handler(request);
		} catch (e) {
			console.error(`Error during handler ${routeKey}`, e);
			return new Response(JSON.stringify({ message: e.toString() }), { status: 404 });
		}
	}
	return new Response(JSON.stringify({ message: `${routeKey} not found` }), { status: 404 });
}
var appResponse = { json: (data) => new Response(JSON.stringify(data), { status: 200 }) };
//#endregion
...
var origin$1 = hasBrowserEnv && window.location.href || "http://localhost";
//#endregion
...
axios.default = axios;
//#endregion
//#region src/api/google/oauth.ts
var googleAuthService = {
	buildAuthUrl: (config) => {
		const query = new URLSearchParams();
		query.append("scope", "https://www.googleapis.com/auth/drive");
		query.append("access_type", "offline");
		query.append("include_granted_scopes", "true");
		query.append("response_type", "code");
		query.append("state", "fewfwefwefwefwefge");
		query.append("redirect_uri", config.web.redirect_uris[0]);
		query.append("client_id", config.web.client_id);
		query.append("prompt", "consent");
		return `https://accounts.google.com/o/oauth2/v2/auth?${query.toString()}`;
	},
	generateToken: async (config, code) => {
		const tokenRequest = new FormData();
		tokenRequest.append("client_id", config.web.client_id);
		tokenRequest.append("client_secret", config.web.client_secret);
		tokenRequest.append("code", code);
		tokenRequest.append("grant_type", "authorization_code");
		tokenRequest.append("redirect_uri", config.web.redirect_uris[0]);
		const now = Date.now();
		const { data: responseData } = await axios.post("https://oauth2.googleapis.com/token", tokenRequest);
		return {
			refreshToken: responseData.refresh_token,
			accessToken: responseData.access_token,
			accessTokenExpires: now + responseData.expires_in * 1e3,
			refreshTokenExpires: now + responseData.refresh_token_expires_in * 1e3
		};
	},
...
//#endregion
//#region src/service/authorizations/authorizations.service.ts
var MINUTE = 1e3 * 60;
var authorizationsService = {
	requireAuthorization: async (authId) => {
		const authorization = await appDb.connection.Authorization.read(authId);
		if (!authorization) throw new Error("Authrization not found");
		return authorization;
	},
	list: () => {
		return appDb.connection.Authorization.list();
	},
	createGoogleAuth: async (request) => {
		console.log("Creating google auth");
		const tokenData = await googleAuthService.generateToken(request.config, request.code);
		const authorization = {
			id: v4(),
			name: request.name,
		...
		await appDb.connection.Authorization.create(authorization);
	},
	getToken: async (authId) => {
		const authorization = await appDb.connection.Authorization.read(authId);
		if (!authorization) throw new Error("Authorization not found");
		if (authorization.type !== AuthorizationType.GOOGLE) throw new Error("Unsupported auth type");
		const tokenData = authorization.tokenData;
		const expireDeadline = Date.now() + MINUTE;
		if (tokenData.accessTokenExpires > expireDeadline) return tokenData.accessToken;
		console.log(/* @__PURE__ */ new Date(), "refreshing auth token for: ", authId);
		const refreshData = await googleAuthService.refreshToken(authorization.config, tokenData.refreshToken);
		const newTokenData = {
			accessToken: refreshData.accessToken,
			accessTokenExpires: refreshData.accessTokenExpires,
		...
		await appDb.connection.Authorization.update(newAuthorization);
		return refreshData.accessToken;
	}
};
//#endregion
//#region src/service/authorizations/authorizations.controller.ts
var authorizationsController = {
	[route("GET", "/api/authorizations")]: async () => {
		const authorizations = await authorizationsService.list();
		return appResponse.json(authorizations);
	},
	[route("POST", "/api/authorizations/google")]: async (request) => {
		const payload = await request.json();
		await authorizationsService.createGoogleAuth(payload);
		return appResponse.json({});
	}
};
//#endregion
//#region src/api/google/gdrive.ts
var gdriveApi = {
	createFolder: async ({ token, name }) => {
		const payload = {
			name,
			mimeType: "application/vnd.google-apps.folder"
		};
		const config = { headers: { Authorization: `Bearer ${token}` } };
		const { data: responseData } = await axios.post("https://www.googleapis.com/drive/v3/files", payload, config);
		return {
			id: responseData.id,
			name: responseData.name
		};
	},
	uploadSimpleFile: async ({ token, parentId, data }) => {
		const boundary = "ax_sw_boundary_12";
		const metadata = {
			name: v4(),
		...
		const { data: { id } } = await axios.post("https://www.googleapis.com/upload/drive/v3/files", mulitipartBody, {
	...
	downloadSimpleFile: async ({ token, fileId }) => {
	...
	updateSimpleFile: async ({ token, fileId, data }) => {
		await axios.patch(`https://www.googleapis.com/upload/drive/v3/files/${fileId}`, data, {
	...
	deleteFile: async ({ token, fileId }) => {
		await axios.delete(`https://www.googleapis.com/drive/v3/files/${fileId}`, { headers: { Authorization: `Bearer ${token}` } });
	}
};
//#endregion
//#region node_modules/@jsonjoy.com/base64/lib/util/strings/flatstr.js
var require_flatstr = /* @__PURE__ */ __commonJSMin(((exports) => {
...
//#endregion
//#region node_modules/@jsonjoy.com/base64/lib/createToBase64.js
var require_createToBase64 = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
...
//#endregion
//#region node_modules/@jsonjoy.com/base64/lib/createFromBase64.js
var require_createFromBase64 = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	...
	var createFromBase64 = (chars = constants_1.alphabet, noPadding = false) => {
		...
		return (encoded) => {
			if (!encoded) return /* @__PURE__ */ new Uint8Array(0);
...
//#endregion
//#region node_modules/@jsonjoy.com/base64/lib/toBase64.js
var require_toBase64 = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.toBase64 = void 0;
	var constants_1 = require_constants();
	var encodeSmall = (0, require_createToBase64().createToBase64)();
...
//#endregion
//#region node_modules/@jsonjoy.com/base64/lib/util/buffers/bufferToUint8Array.js
var require_bufferToUint8Array = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
...
//#endregion
//#region node_modules/@jsonjoy.com/base64/lib/fromBase64.js
...
//#endregion
//#region src/service/app.crypto.ts
...
var AppKey = class AppKey {
	...
	encryptJson(data) {
		...
		const textData = new TextEncoder().encode(text).buffer;
		return this.encrypt(textData);
	}
	decrypt(data) {
	...
	async decryptJson(data) {
		const textData = await this.decrypt(data);
		const text = new TextDecoder().decode(textData);
		return JSON.parse(text);
	}
	async save() {
		return `${(0, import_lib.toBase64)(new Uint8Array(await self.crypto.subtle.exportKey("raw", this.key)))}$$$${(0, import_lib.toBase64)(this.iv)}`;
	}
	static async loadKey(data) {
		const parsedData = data.split("$$$");
		const keyData = (0, import_lib.fromBase64)(parsedData[0]);
		const iv = (0, import_lib.fromBase64)(parsedData[1]);
		return new AppKey(await self.crypto.subtle.importKey("raw", keyData.buffer, {
			name: "AES-GCM",
			length: 256
		}, true, ["encrypt", "decrypt"]), iv);
	}
	static async generate() {
		const iv = self.crypto.getRandomValues(/* @__PURE__ */ new Uint8Array(12));
		return new AppKey(await self.crypto.subtle.generateKey({
			name: "AES-GCM",
			length: 256
		}, true, ["encrypt", "decrypt"]), iv);
	}
...
//#endregion
//#region src/service/vaults/vaults.service.ts
var vaultsCache = /* @__PURE__ */ new Map();
async function loadIndexNode(vault) {
	const token = await authorizationsService.getToken(vault.authorizationId);
	const indexData = await gdriveApi.downloadSimpleFile({
		token,
		fileId: vault.params.indexFile
	});
	return (await AppKey.loadKey(vault.masterKey)).decryptJson(indexData);
}
var vaultsService = {
	requireVault: async (vaultId) => {
		const vault = await appDb.connection.Vault.read(vaultId);
		if (!vault) throw new Error("Vault not found");
		return vault;
	},
	parseVault: async (vaultId) => {
		const parsed = vaultsCache.get(vaultId);
		if (parsed) return parsed;
		const vault = await vaultsService.requireVault(vaultId);
		const newParsed = {
			index: await loadIndexNode(vault),
			vault
		};
		vaultsCache.set(vault.id, newParsed);
		return newParsed;
	},
	generateVault: async ({ authorizationId, type, name }) => {
		if (type !== VaultType.GDRIVE) throw new Error("Unsupported vault type");
		const token = await authorizationsService.getToken(authorizationId);
		const rootFolder = await gdriveApi.createFolder({
			token,
			name: `.wow_${Date.now()}`
		});
		const masterKey = await AppKey.generate();
		const indexNode = {
			version: 1,
			nodes: []
		};
		const indexFile = await gdriveApi.uploadSimpleFile({
			token,
			parentId: rootFolder.id,
			data: await masterKey.encryptJson(indexNode)
		});
		const vault = {
			id: v4(),
			type: VaultType.GDRIVE,
			name,
			createdAt: Date.now(),
			masterKey: await masterKey.save(),
			authorizationId,
			params: {
				rootFolder,
				indexFile
		...
		await appDb.connection.Vault.create(vault);
		vaultsCache.set(vault.id, {
			vault,
			index: indexNode
		});
		return vault.id;
	...
	deleteVault: async (vaultId) => {
		const vault = await vaultsService.requireVault(vaultId);
		const token = await authorizationsService.getToken(vault.authorizationId);
		vaultsCache.delete(vaultId);
		await gdriveApi.deleteFile({
			token,
			fileId: vault.params.rootFolder.id
		});
		await appDb.connection.Vault.delete(vaultId);
	}
};
//#endregion
//#region src/service/nodes/nodes.service.ts
function listNodes(index, parentId) {
	return index.nodes.filter((it) => parentId && it.parentId === parentId || !parentId && !it.parentId);
}
function getNodePath(index, nodeId) {
	const result = [];
	let currentNodeId = nodeId;
	while (currentNodeId) {
		const currentNode = index.nodes.find((it) => it.id === currentNodeId);
		if (!currentNode) throw new Error(`Inconsistent index cannot find path for ${nodeId}`);
		if (currentNode.type !== "DIR") throw new Error(`Inconsistent index: not dir node in parent ${nodeId}`);
		result.push(currentNode);
		currentNodeId = currentNode.parentId;
	}
	(0, import_lodash.reverse)(result);
	return result;
}
var nodesService = {
	listNodes: async (payload) => {
		const { index } = await vaultsService.parseVault(payload.vaultId);
		const nodes = listNodes(index, payload.parentId);
		return {
			path: getNodePath(index, payload.parentId),
			nodes
		};
	},
	uploadFile: async (payload) => {
		const { parentId, file, vaultId } = payload;
		const { vault, index } = await vaultsService.parseVault(vaultId);
		const token = await authorizationsService.getToken(vault.authorizationId);
		const fileKey = await AppKey.generate();
		const content = await file.arrayBuffer();
		const fileId = await gdriveApi.uploadSimpleFile({
			parentId: vault.params.rootFolder.id,
			token,
			data: await fileKey.encrypt(content)
		});
		const fileNode = {
			id: v4(),
			type: "FILE",
		...
		const newIndex = {
			...
			nodes: [...index.nodes, fileNode]
		};
		const masterKey = await AppKey.loadKey(vault.masterKey);
		await gdriveApi.updateSimpleFile({
			token,
			fileId: vault.params.indexFile,
			data: await masterKey.encryptJson(newIndex)
		});
		index.nodes.push(fileNode);
	},
	createDirNode: async (request) => {
		const { parentId, name, vaultId } = request;
		const { vault, index } = await vaultsService.parseVault(vaultId);
		const token = await authorizationsService.getToken(vault.authorizationId);
		const dirNode = {
			id: v4(),
		...
		index.nodes.push(dirNode);
		return dirNode.id;
	}
};
//#endregion
//#region src/service/nodes/nodes.controller.ts
var nodesController = {
	[route("POST", "/api/nodes/list")]: async (request) => {
		const payload = await request.json();
		const response = await nodesService.listNodes(payload);
		return appResponse.json(response);
	},
	[route("POST", "/api/nodes/dir/create")]: async (request) => {
		const payload = await request.json();
		const id = await nodesService.createDirNode(payload);
		return appResponse.json({ id });
	},
	[route("GET", "/api/nodes/file/download")]: async (request) => {
		const url = new URL(request.url);
		const { vault, index } = await vaultsService.parseVault(url.searchParams.get("vaultId"));
		const node = index.nodes.find((it) => it.id === url.searchParams.get("nodeId"));
		if (!node) throw new Error("Node not found");
		if (node.type !== "FILE") throw new Error("Cannot download not a file");
		const token = await authorizationsService.getToken(vault.authorizationId);
		const content = await gdriveApi.downloadSimpleFile({
			token,
			fileId: node.storageId
		});
		const decrypted = await (await AppKey.loadKey(node.key)).decrypt(content);
		return new Response(decrypted, { headers: {
			"Content-Type": "application/octet-stream",
			"Content-Disposition": `attachment; filename="${encodeURIComponent(node.name)}"`
		} });
	},
	[route("POST", "/api/nodes/file/upload")]: async (request) => {
		const formData = await request.formData();
		const file = formData.get("file");
		const vaultId = formData.get("vaultId");
		const parentId = formData.get("parentId");
		if (!file) throw new Error("File not found in request");
		if (!vaultId) throw new Error("Vault not found in request");
		await nodesService.uploadFile({
			parentId,
			file,
			vaultId
		});
...
//#endregion
//#region src/service/vaults/vaults.controller.ts
var vaultsController = {
	[route("GET", "/api/vaults")]: async () => {
		const vaults = await appDb.connection.Vault.list();
		return appResponse.json(vaults);
	},
	[route("POST", "/api/vaults")]: async (request) => {
		const payload = await request.json();
		const id = await vaultsService.generateVault(payload);
		return appResponse.json({ id });
	},
	[route("DELETE", "/api/vaults")]: async (request) => {
...
//#endregion
//#region src/service/app.controller.ts
var appController = { handle: (request) => performRouting([
	vaultsController,
	nodesController,
...
var BASE_URL = "/test-pages/".endsWith("/") ? "/test-pages/".slice(0, -1) : "/test-pages/";
self.addEventListener("fetch", (event) => {
	if (!event.request.url.replace(origin, "").startsWith(`${BASE_URL}/api`)) return;
	event.respondWith(appController.handle(event.request));
});
//#endregion
